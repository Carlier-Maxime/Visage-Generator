...
class VisageGenerator:
    ...
    def save_batch(self, cfg: Config, leave_pbar: bool = True):
        for i in trange(len(self._vertices), desc='saving batch', unit='visage', leave=leave_pbar):
            ...
            self.savers.png(index, basename, vertices, texture, vertical_flip=cfg.save.global_.vertical_flip, depth_in_alpha=cfg.save.depth.alpha)
            ...
            self.savers.depth(index, basename, vertices, texture, pts=markers, ptsInAlpha=cfg.save.depth.alpha, vertical_flip=cfg.save.global_.vertical_flip, save_depth=True)
