...
class VisageGenerator:
    ...
    def save_batch(self, cfg: Config, leave_pbar: bool = True):
        for i in trange(len(self._vertices), desc='saving batch', unit='visage', leave=leave_pbar):
            ...
            self.markers_png_Saver(index, basename + '.png', vertices, texture, pts=util.read_all_index_opti_tri(vertices, self._faces, self.markers) if self.markers_png_Saver.enable else None, ptsInAlpha=cfg.pts_in_alpha, camera=camera, vertical_flip=cfg.vertical_flip)
...
@click.option('--camera-params', type=str, metavar=float, default=[1, 8, 12, 2, -0.05, 0.05, 1, -2.1, -1.9, 3, -30, 30], help='Camera parameter intervals. Format: [n1,min1,max1,n2,min2,max2,...]. sum(nX)==7, params order : [fov, tx, ty, tz, rx, ry, rz]. (rotation in degree)', callback=click_callback_str2list)
